 ...
 lcd.print("US");
  ...
  if(Distance1>=10)
  {
    if(IR1_Val==1) //Now, I am in the living room.
    {
      ...
      while(digitalRead(hitA))
      {
        //do nothing and wait for the master coming
      }
      ...
      while(digitalRead(hitA))
      {
      //do nothing and wait for the master who come back to the living room after finished the lunch.
      }
      ...
      while(digitalRead(hitA))
      {
        //Here, do nothing and wait for the master who coming back from outside
        //Still waiting for him to take rest and finish the bathe 
      }
      ...
      while(digitalRead(hitA))
      {
      //do nothing and waiting for the human coming back from dining room to living room
      }
  ...
  if(Distance1>=10)
  {
    if(IR1_Val==1)  //Now, I am in the bedroom again.
    {
      ...
      while(digitalRead(hitA))
      {
      //do nothing and waiting for the human coming back from living room to dining room
      }
...
/*Interupt to stop the morning music. 
Here, we assume this function as the camera to sense that 
human finishs the breakfast. So, target_b is hit and stop the music*/
ISR(PCINT2_vect) 
{
  if (!digitalRead(hitB))
